...
func sendMessage(evValue string) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("socketServer MRS error", r)
		}
	}()
	// mrs 서버의 ip와 temporary port. port는 mrs의 system.properties에서 정함.
	conn, err := net.Dial("tcp", config.Config("socketServer"))
	if nil != err {
	...
	headerA := fmt.Sprintf("%3v", config.Config("clientCD")) + //
		fmt.Sprintf("%8v", config.Config("siteCD")) + //
		"A1" +
		"      SIM" +
		" " // message exchange pattern

	headerB := "001" + // message type cd
		util.UUID24Digit() + // trace id
	...
	bodyJson.StatEvet.USvcOutbId = currentDateTimeString
